...
if(event.keyCode==13) {
  ...
  return (
    ...
    <div className=' flex flex-row justify-center items-center mt-[20px] gap-[10px]'>
        <motion.input initial={{x:-30,opacity:0}} animate={{x:0,opacity:1}} transition={{duration:1.5,delay:1}}  className=' md:w-[250px] w-[180px] h-[30px] py-[4px] px-[4px] outline-none rounded-lg bg-black placeholder:text-orange-600 text-orange-600 font-bold' type="text" placeholder='Search...' onChange={handleSearch} value={searchQuery} onKeyDown={handleEnter}/>
        <motion.button initial={{x:30,opacity:0}} animate={{x:0,opacity:1}} transition={{duration:1.5,delay:1}} >
        <FaSearch className='text-[20px] mt-[2px] text-black cursor-pointer'  onClick={getSearchData}/>
        </motion.button>
